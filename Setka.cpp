...
void Setka::M_K_prepare(void)
{
	...
	double ss2 = 0.02; // 0.007; // * 0.5;    0.03
...
// to_I - в какую область предназначался атом
{
	...
	if (true)//(ExCh == false) // Если перезарядки в этой ячейке ещё не было
	{
		...
		//if (r < 100.0/RR_ && r > 60.0 / RR_)// && fabs(mu) > Mu[area][now->zona] * mu_start * 0.3 * sin(all))
		//{
		//	bool sec = false;
		//	bool sec2 = false;
		//	double tt;
		...
		//	double a3 = kvv(x_0, y_0, z_0) - kv(80.0 / RR_);
		...
		//		if (sec2)
		//		{
		//			this->mu_stat[this->number_stat] = mu;
		//		}
		//		else
		//		{
		//			this->mu_stat[this->number_stat] = mu2;
		//		}
		//		this->number_stat++;
		//		mut_stat.unlock();
		//	}
